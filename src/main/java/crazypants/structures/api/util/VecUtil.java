...
public class VecUtil {
  ...
  public static Collection<Point3i> getTaggedLocationsInWorldCoords(IStructureTemplate template, String target, int originX, int originY, int originZ, Rotation rotation) {
    List<Point3i> res = new ArrayList<Point3i>();
    for(IStructureComponent comp : template.getComponents()) {
      List<Point3i> locs = comp.getTaggedLocations(target);
      for(Point3i p : locs) {
        Point3i xFormed = new Point3i(p);
        xFormed = VecUtil.transformStructureCoodToWorld(originX, originY, originZ, rotation, comp.getSize(), xFormed);
        res.add(xFormed);
      }
    }
    return res;
